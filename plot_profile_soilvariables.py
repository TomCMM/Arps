#===============================================================================
# DESCRIPTION
#    Plot the profile of the variables in the soil
#===============================================================================
...
#     sims = [ 's5oc']
    ...
    path = "/dados3/soc/s5oc/out9kmk91/"
    ...
    varnames = ['QSOIL']# does not have the same shape need to be implemented
...
# pt = Serie.getdfmap('TSOIL', Iselect=[[0,1],[0,1],[0,10],[0,1],[0,1] ])
# 
# print pt
...
#     Serie = netcdf_serie(Files,'ARPS')
    ...
    for staname, i,j in zip(attributes.index, idx['i'].values, idx['j'].values):
        height[staname] = ARPS.get('ZPSOIL', Iselect=[[0,10], [i,i+1], [j,j+1]]).flatten()
        data[staname]={}
        for var in varnames:
            data[staname][var] = Serie.getdfmap(var, Iselect=[[0,1],[0,1], [0,10], [i,i+1], [j,j+1] ])
#     
#         print height
    for staname in attributes.index:
        for var in varnames:
            df = data[staname][var].groupby(lambda x: (x.hour)).mean()
            fig = plt.figure()
